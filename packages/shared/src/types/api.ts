...
// ── Credential choice prompt (server-triggered disambiguation) ─

export interface CredentialChoiceCandidate {
  id: string;
  name: string;
  type: string;
  usageContext?: string;
...
export interface CredentialChoiceRequestData {
  requestId: string;
  sessionId: string | null;
  credentialType: string;
  toolName?: string;
  reason: string;
  candidates: CredentialChoiceCandidate[];
  timeoutMs: number;
  timestamp: string;
}

export type CredentialChoiceAction = "use-once" | "pin-session" | "cancel";

export interface CredentialChoiceResponseData {
  requestId: string;
  action: CredentialChoiceAction;
  credentialId?: string;
}
...
export type GatewayClientEvent =
  | {
      ...
      data: {
        ...
        workspaceId: string;
        inputModality?: "voice";
        media?: import("./message.js").MediaArtifact[];
        messageId?: string;
      };
    }
  ...
  | { type: "credential.choice.respond"; data: CredentialChoiceResponseData }
...
export type GatewayServerEvent =
  ...
  | {
      ...
      data: import("./execution.js").ExecutionEvent;
  ...
  | { type: "credential.choice.request"; data: CredentialChoiceRequestData }
  ...
  | {
      ...
      data: { status: "connecting" | "connected" | "disconnected"; phoneNumber?: string };
    }
  | { type: "skills.reloaded"; data: Record<string, never> }
  | {
      type: "registry.updatesAvailable";
      data: { count: number; skills: { id: string; current: string; available: string }[] };
    }
