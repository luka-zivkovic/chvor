---
name: Claude Code
description: Delegate complex coding tasks to the Claude Code CLI agent
version: 1.0.0
category: developer
icon: terminal
---
...
- Tasks that don't involve source code

## How to use

Call `native__claude_code` with:

- **prompt** (required): Be specific. Include file paths, expected behavior, constraints.
...
- Claude Code works best with clear, scoped tasks — one logical change per invocation
- For large refactors, break the work into steps and call Claude Code multiple times
- Always review the result before presenting it to the user
- If a task times out, try breaking it into smaller pieces or increasing `maxTurns`
