    ...
    return {
      content: [{
        ...
        text: `Integration researched: ${proposal.name}\n` +
          ...
          `Next steps: (1) native__request_credential with source "ai-research" and confidence "${proposal.confidence}" to collect the credential, ` +
          `then (2) native__synthesize_tool with credentialType="${proposal.credentialType}", baseUrl="${proposal.baseUrl ?? ""}", ` +
          `authScheme="${proposal.authScheme ?? "bearer"}"${proposal.specUrl ? `, openApiSpecUrl="${proposal.specUrl}"` : " (provide endpoints[] if no spec)"} ` +
...
async function handleRequestCredential(
  ...
  return {
    content: [{
      type: "text",
      text: `Credential "${credName}" (${credentialType}) saved (id: ${savedId}).${testMsg}${nextStepMsg}`,
    }],
  };
}
...
async function handleSynthesizeTool(
  args: Record<string, unknown>,
): Promise<NativeToolResult> {
...
async function handleRepairSynthesizedTool(
  args: Record<string, unknown>,
  context?: NativeToolContext,
): Promise<NativeToolResult> {
...
handlers.set(SYNTHESIZE_TOOL_NAME, handleSynthesizeTool);
nativeToolMapping.set(SYNTHESIZE_TOOL_NAME, { kind: "tool", id: "credentials" });
handlers.set(REPAIR_SYNTHESIZED_TOOL_NAME, handleRepairSynthesizedTool);
nativeToolMapping.set(REPAIR_SYNTHESIZED_TOOL_NAME, { kind: "tool", id: "credentials" });
